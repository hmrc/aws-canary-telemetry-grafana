...
debug_env(){
  ...
  echo S3_TELEMETRY_CANARY_ROOT="${S3_TELEMETRY_CANARY_ROOT}"
...
# Prepare dependencies and build the Canary function code using Docker
package() {
  print_begins
...
# Take all the necessary steps to build and publish both canary function's zip and checksum files
publish() {
  print_begins
...
publish_artifacts_to_s3() {
  ...
  aws s3 cp "${PATH_BUILD}/${CANARY_ZIP_NAME}" "${S3_ADDRESS}/${PROJECT_FULL_NAME}.${VERSION}.zip" \
...
help() {
  ...
  echo -e " - package\t\t\t Prepare dependencies and build the Canary function code using Docker"
  ...
  echo -e " - publish_checksum_file\t Generate a checksum for the artifacts zip file and store in the same S3 location (${S3_CANARY_SUB_FOLDER})"
...
print_configs() {
  ...
  echo
  echo -e "S3_TELEMETRY_CANARY_ROOT:\t${S3_TELEMETRY_CANARY_ROOT}"
  echo -e "S3_CANARY_SUB_FOLDER:\t\t${S3_CANARY_SUB_FOLDER}"
